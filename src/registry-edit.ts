 ...
 */
function filterId(list: unknown, sessionId: string): unknown {
...
export function filterWorkspaceState(
  ...
  sessionId: string,
  mode: 'delete' | 'restore',
): WorkspaceStateShape | null {
  if (typeof state !== 'object' || state === null) return null
  const next = clone(state as WorkspaceStateShape)
  let changed = false
 ...
 */
export function workspaceJsonAfter(text: string, sessionId: string, mode: 'delete' | 'restore'): string | null {
  let doc: { global?: { archivedSessionIds?: unknown }; tables?: { workspaces?: Record<string, { sessionIds?: unknown }> } }
  try {
    doc = JSON.parse(text)
  } catch {
    return null
  ...
  if (mode === 'delete') {
    ...
    if (workspaces) {
      for (const record of Object.values(workspaces)) {
        if (record && 'sessionIds' in record) record.sessionIds = filterId(record.sessionIds, sessionId)
      }
    }
  }
 ...
 * @returns the rewritten document text, or null when the input is invalid.
 */
export function projcacheJsonAfter(text: string, sessionId: string): string | null {
  let doc: { tables?: { sessions?: Record<string, unknown> } }
  try {
    doc = JSON.parse(text)
  } catch {
    return null
